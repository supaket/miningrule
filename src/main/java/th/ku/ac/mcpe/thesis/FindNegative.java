...
public class FindNegative {
  ...
  public void parsePositiveFile(String path) {
    ...
    try {
      ...
      while (in.ready()) {
        ...
        if (isFirstLine) {
          isFirstLine = false;
          seqCount = Integer.valueOf(line.replace('(', ' ').replace(')', ' ').trim());
        }
        Matcher matcher = Pattern.compile("^(\\d+) +(\\((\\d+)\\))\t([0|1]+)$").matcher(line);
        ...
        if (matcher.find()) {
          ...
          // NS
          Matcher nslMatcher = Pattern.compile("^((\\d+ +)+)\\((\\d+)\\)\t([0|1]+)$").matcher(line);
          if (nslMatcher.find()) {
            revertNSL1Count++;
  ...
  public String revertL1(int seqCount, Matcher matcher) {
    String neg =
        ("~" + matcher.group(1)) + " " + "(" + String.valueOf(seqCount - Integer.valueOf(matcher.group(3))) + ")" + "\t" + revertTrnxSeq(matcher.group(4));
    return neg;
  }
